# Panduan Upload Data Simpanan Pokok

## Fitur Baru: Upload CSV/Excel untuk Simpanan Pokok

Fitur ini memungkinkan administrator untuk menginput data simpanan pokok secara batch (banyak anggota sekaligus) menggunakan file CSV atau Excel.

## Cara Menggunakan

Ada **2 cara** untuk upload data simpanan pokok:

### Cara 1: Upload File CSV/Excel

#### 1. Download Template

1. Buka menu **Simpanan** → **Simpanan Pokok**
2. Klik tombol **"Download Template"** (ikon download)
3. File `template_simpanan_pokok.csv` akan terdownload
...
- **Jumlah**: Jumlah simpanan pokok dalam angka (bisa pakai format apapun, sistem akan bersihkan otomatis)
- **Tanggal**: Tanggal transaksi - **SISTEM OTOMATIS KONVERSI** berbagai format:
  - ✅ `2024-01-15` (YYYY-MM-DD) - Format standar
...
1. ✅ Data simpanan pokok tersimpan untuk setiap anggota
2. ✅ Jurnal akuntansi otomatis tercatat:
   - Debit: Kas (1-1000)
   - Kredit: Simpanan Pokok (2-1100)
3. ✅ Data muncul di tabel Simpanan Pokok

## Tips

### Tips Umum
1. **Periksa NIK**: Pastikan semua NIK sudah terdaftar di sistem sebelum upload
2. **Format Tanggal**: Gunakan format YYYY-MM-DD (contoh: 2024-01-15, bukan 15/01/2024)
...
3. **Jumlah Fleksibel**: Bisa pakai format apapun (1000000 atau 1.000.000), sistem akan bersihkan otomatis
...
### Contoh 2: Data dari Excel (untuk Paste)
...
2. Ctrl+C
3. Buka sistem → Upload Data → Tab "Paste Data"
4. Ctrl+V di text box
5. Klik "Preview Data"
...
> **Note**: Sistem otomatis deteksi apakah delimiter pakai koma (,) atau tab
...
- Tidak ada titik, koma, atau karakter lain
...
- Awal periode koperasi (input simpanan pokok semua anggota)
- Migrasi data dari sistem lama
- Event pendaftaran anggota massal
